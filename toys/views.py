...
# Usando las clases de visualización por métodos que son elaboradadas "manualmente"
...
def toy_list(request):
    if request.method == "GET":
        ...
        return JSONResponse(toy_serializer.errors, status.HTTP_400_BAD_REQUEST)
...
def toy_detail(request, pk):
    ...
    if request.method == "GET":
        ...
        toy_data = JSONParser().parse(request)
        toy_serializer = ToySerializer(toy, data=toy_data, partial=True)
        if toy_serializer.is_valid():
            toy_serializer.save()
            return JSONResponse(toy_serializer.data)
        return JSONResponse(toy_serializer.errors, status.HTTP_400_BAD_REQUEST)
